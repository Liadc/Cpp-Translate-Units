#include "PhysicalNumber.h"
#include <stdexcept>
#include <iostream>
#include <cmath>


using namespace std;
using namespace ariel;

PhysicalNumber::PhysicalNumber(double units, Unit typeID){
    this->typeID = typeID;
    this->units = units;
};

Unit PhysicalNumber::getTypeID(){
    return this->typeID;
};

double PhysicalNumber::getUnits(){
    return this->units;
};

void PhysicalNumber::setUnits(double units){
    this->units = units;
};

double PhysicalNumber::translateOther(const PhysicalNumber& other){
    if(this->typeID == other.typeID){ //same types. maybe its not even needed.
        return other.units;
    }
    int dif = ((int)this->typeID - (int)other.typeID);
    if(dif % 3 != 0){
        throw runtime_error("Cannot convert");
    } else{
        dif /= 3;
        switch((int)other.typeID % 3){
            case 0:case 1: //Length or Mass cases
                return other.units*(pow(1000,(-dif)));
                break;
            case 2: //Time case
                return other.units*(pow(60,(-dif)));
                break;
            }
    }
};

PhysicalNumber& PhysicalNumber::operator+(const PhysicalNumber& other){ //add units same world
    PhysicalNumber a(this->units+translateOther(other), this->typeID);
    return a;
};

PhysicalNumber& PhysicalNumber::operator-(const PhysicalNumber& other){ //decrease units same world
    PhysicalNumber a(this->units-translateOther(other), this->typeID);
    return a;
};

//adding  +=
PhysicalNumber& PhysicalNumber::operator+=(const PhysicalNumber& other){
    setUnits(this->units + translateOther(other));
    return *this;
};

PhysicalNumber& PhysicalNumber::operator-=(const PhysicalNumber& other){
    setUnits(this->units - translateOther(other));
    return *this;
};

//unary plus
PhysicalNumber& PhysicalNumber::operator+(){
    return *this;
};

//unary minus
PhysicalNumber& PhysicalNumber::operator-(){
    PhysicalNumber a(-this->units, this->typeID);
    return a;
};

bool PhysicalNumber::operator>(const PhysicalNumber& other){
    return this->units > translateOther(other);
 };

bool PhysicalNumber::operator>=(const PhysicalNumber& other){
    return this->units >= translateOther(other);
 };

bool PhysicalNumber::operator<(const PhysicalNumber& other){
    return this->units < translateOther(other);
};

bool PhysicalNumber::operator<=(const PhysicalNumber& other){
    return this->units <= translateOther(other);
 };

bool PhysicalNumber::operator==(const PhysicalNumber& other){
    return this->units == translateOther(other);
 };

bool PhysicalNumber::operator!=(const PhysicalNumber& other){
    return this->units != translateOther(other);
 };

ostream& ariel::operator<<(ostream& os, PhysicalNumber pn){
    const char *types[] = { "cm","h","sec","m","kg","min","km","ton","hour" };
    return os << pn.getUnits() <<"[" << types[(int)pn.typeID] << "]";
 };
            
istream& ariel::operator>>(istream& is, PhysicalNumber pn){
    //update needed with regex.
    return is;
};